        ...
        <span className="mt-2 flex flex-col max-w-inherit w-[100%]">
            <header className={`w-inherit flex border-l-[5px] ${left}`}>
                <span className= {`${border} h-[1.5rem] px-2`}>
                    <h1 className="text-md">{keywords != null ? keywords[0].toUpperCase() : "Top News"}</h1>
                </span>
            </header>

            <div className="search-box h-[9rem] flex overflow-hidden">
                <img src={image_url ? posterImage : generic} onClick={() => navigateToArticlePage(navigate,title,data)}   className="searchquery-img h-inherit max-w-[10.938rem] w-[100%] hover:cursor-pointer" alt="news" />
                <aside  className="ml-8 flex flex-col text-[17px] max-[1069px]:text-md hover:cursor-pointer">
